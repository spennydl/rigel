        ...
        for (usize sc = 0; sc < stage_colliders->n_aabbs; sc++) {
            ...
            if (collision_result.t_to_collide >= 0) {
                if (collision_result.t_to_collide > 0) {
                    ...
                    new_pos = entity->position + ((collision_result.t_to_collide - buffertime) * displacement);
                } else {
                    auto correction = collision_result.penetration_axis * (collision_result.depth + 1);
                    entity->position += correction;
                    // move out this frame and don't try to move more?
                    // or should we move the whole system out of a wall?
                    // try this first
                    new_pos += correction;
                }
                ...
                displacement = new_pos - entity->position;
                collider.center = entity->position + collider.extents;
        ...
        for (int t = 0; t < stage_colliders->n_tris; t++) {
            ...
            if (collision_result.t_to_collide >= 0) {
                ...
                if (collision_result.t_to_collide > 0) {
                    ...
                    auto correction = collision_result.vdist_to_out + 1;
